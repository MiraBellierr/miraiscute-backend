...
function generateSitemap(db, publicDir = null) {
  try {
    ...
    try {
      const videos = db.prepare('SELECT id, createdAt FROM videos ORDER BY createdAt DESC LIMIT 100').all();
      if (videos && Array.isArray(videos)) {
        videos.forEach((video) => {
          entries.push({
            url: `${WEBSITE_BASE}/videos#${video.id}`,
      ...
      console.warn('Could not fetch videos for sitemap:', err.message);
    }

    // Fetch and add pics
    try {
      const pics = db.prepare('SELECT id, createdAt FROM pics ORDER BY createdAt DESC LIMIT 100').all();
      if (pics && Array.isArray(pics)) {
        pics.forEach((pic) => {
          entries.push({
            url: `${WEBSITE_BASE}/pics#${pic.id}`,
      ...
      console.warn('Could not fetch pics for sitemap:', err.message);
    }
